...
CameraControl::CameraControl(Entity::reference camera) noexcept
	: _camera(camera)
...
void CameraControl::onRender() noexcept {
    auto posDelta = _velocity * glm::mat3(_camera->transform.getModelMat()) * _direction;
    auto position = _camera->transform.getPosition() + posDelta;

	_camera ->transform.setRotate(_eilerAngle);
